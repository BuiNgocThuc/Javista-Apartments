...
public class MomoServiceImpl implements MomoService {
    ...
    @Override
    public MomoPaymentResponse createPayment(Bill bill, MomoPaymentCreationRequest request) throws Exception {
        ...
        // Create extraData
        String extraData =
                Base64.getEncoder().encodeToString(objectMapper.writeValueAsBytes(new ExtraData(bill.getId())));

        // Build rawData
        String rawData = String.format(
                "accessKey=%s&amount=%.0f&extraData=%s&ipnUrl=%s" + "&orderId=%s&orderInfo=%s&partnerCode=%s"
        ...
        try {
            ...
            ResponseEntity<MomoPaymentResponse> response = restTemplate.exchange(
                    momoConfig.getMomoApiUrl(), HttpMethod.POST, httpEntity, MomoPaymentResponse.class);
